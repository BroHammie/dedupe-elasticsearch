...
afterAll(() => {
  return Promise.all([
    ...
    deleteIndex(client, quadrupleIndex),
  ]);
});
...
it('double seed', async () => {
  ...
  await seedNested(client, doubleIndex);
  const duplicates = await getAllDuplicates(client, doubleIndex, ['character.firstName', 'quote']);
...
it('3 seed', async () => {
  ...
  await seedNested(client, tripleIndex);
  await seedNested(client, tripleIndex);
  const duplicates = await getAllDuplicates(client, tripleIndex, ['character.firstName', 'quote']);
  Object.values(duplicates).forEach(value => {
...
it('4 seed', async () => {
  ...
  await seedNested(client, quadrupleIndex);
  await seedNested(client, quadrupleIndex);
  const duplicates = await getAllDuplicates(client, quadrupleIndex, ['character.firstName', 'quote']);
